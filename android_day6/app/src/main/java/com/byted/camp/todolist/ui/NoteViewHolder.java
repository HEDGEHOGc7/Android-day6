    ...
    public void bind(final Note note) {
        ...
        editBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int newp =  0;
                note.setnewPriority(newp);

                   operator.editNote(note);

                Log.i(TAG, " pro intvalue"+note.getPriority().intValue+" state "+note.getState().intValue);
            }
        });
        ...
        if(note.getPriority().intValue==0){
            itemView.setBackgroundColor(Color.WHITE);
        }else if(note.getPriority().intValue==1){
            itemView.setBackgroundColor(Color.GREEN);
        }else if(note.getPriority().intValue==2){
